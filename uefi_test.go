...
func TestAppendUEFIFirmwareDeviceArch(t *testing.T) {
	udev := UEFIFirmwareDevice{Code: "AAVMF_CODE.ms.fd", Vars: "AAVMF_VARS.ms.fd"}
	expected := "-drive if=pflash,format=raw,readonly=on,file=AAVMF_CODE.ms.fd -drive if=pflash,format=raw,file=AAVMF_VARS.ms.fd"
	testAppend(udev, expected, t)
}

func TestAppendUEFIFirmwareDeviceConfigArch(t *testing.T) {
	c := &Config{}
	udev := UEFIFirmwareDevice{Code: "AAVMF_CODE.ms.fd", Vars: "AAVMF_VARS.ms.fd"}
	c.UEFIFirmwareDevices = append(c.UEFIFirmwareDevices, udev)
	err := c.appendDevices()
...
func TestNewFirmwareDev(t *testing.T) {
	udev_ptr, err := NewSystemUEFIFirmwareDevice(true)
	if err != nil {
	...
	case "arm64", "aarch64":
		expected := "-drive if=pflash,format=raw,readonly=on,file=/usr/share/AAVMF/AAVMF.ms.fd -drive if=pflash,format=raw,file=/usr/share/AAVMF/AAVMF_VARS.ms.fd"
		testAppend(udev, expected, t)
	}
}
